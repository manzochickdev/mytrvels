...
import kr.changhan.mytravels.dao.TravelExpenseDao;
...
public class TravelDetailRepository {
    ...
    // expense
    public LiveData<PagedList<TravelExpense>> getAllExpensesOfTravel(long travelId, String dateTime,String currency) {
        if (MyString.isEmpty(dateTime) && MyString.isEmpty(currency))
            return new LivePagedListBuilder<>(mTravelExpenseDao.getAllExpensesOfTravel(travelId), 20).build();
        else if (MyString.isEmpty(dateTime) && MyString.isNotEmpty(currency)){
            return new LivePagedListBuilder<>(mTravelExpenseDao.getExpensesOnCurrency(travelId,currency), 20).build();
        }
